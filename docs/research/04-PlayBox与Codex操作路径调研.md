...
本轮已收到 Playbox 3 份与大洋 5 份真实日单。两套系统分管不同频道，导入导出验证分别按[验证计划 E17](../planning/02-前期探索与验证计划.md)执行；当前未取得双向客户端验证。资料统一按[传媒调研报告](../research/09-传媒项目立项前调研与基础分析报告.md)，已有文件不重复索取。下文公开产品能力为历史调研，不作为现场版本支持证明。
...
现场资料只按[传媒调研报告](../research/09-传媒项目立项前调研与基础分析报告.md)的适用范围收集：Playbox 为完整实例，大洋只补独立导入导出所需材料。文件字段和缺口由项目侧先检查；远程路线仍需对应软件与测试权限。
