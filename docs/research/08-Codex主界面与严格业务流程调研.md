...
结论：Codex 继续作为工作人员操作入口。被调查项目采用的机制从流程提示、文件交接、执行拦截，逐步扩展到程序化状态流转和持久恢复。提示词完整、声称某步骤“必须执行”、拥有高星数或提供固定 JSON，均不能单独证明业务流程完整。锦书应验证的是“必经步骤有实际结果、结果属于当前版本、缺条件不能正常交付”。
...
本轮读取的核验操作先查执行尝试是否已结算成功且处于允许的核验阶段，再接收带环境信息的核验结果，并检查是否已有权威结果；接口还包含重复请求标识与版本条件。检查依据变化后，可使旧的通过结果失效，避免继续使用。
...
4. **业务确认与恢复有记录。**歧义返回可理解的问题，回答关联当前条目和版本；退出后返回，或对话过长被压缩整理时，重新读取已保存的任务状态。旧选择仍有效才复用，重试先查上一次实际结果。
