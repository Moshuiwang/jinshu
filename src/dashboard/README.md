...
本目录是唯一可运行设计原型，`dist/` 内文件直接维护并受 Git 管理，无生成步骤；不是临时预览缓存。公司与系统 Logo 随 `dist/assets/` 入库。现行设计来源为 [PR #10](https://github.com/Moshuiwang/jinshu/pull/10)／[PR #11](https://github.com/Moshuiwang/jinshu/pull/11)，演示模型与后续真实状态源必须显式分开。

- [设计基线](../../docs/dashboard/02-视觉设计基线.md)：布局、颜色、动效与版本依据。
- [状态协同需求](../../docs/dashboard/01-大屏需求.md)：由大屏反推的功能。
- [工程与数据库设计](../../docs/dashboard/03-工程与数据库设计.md)：模块、数据表与供数接口。
- [内网部署方案](../../docs/dashboard/04-内网部署方案.md)：依赖、离线包与恢复。
- [联网开发核查](../../docs/dashboard/05-开发核查.md)：官方依据、文档修正与未验证项。
- [本地实验索引](../../outputs/README.md)：早期图稿和样式选择记录，仅用于追溯。
...
打开[浅色预览](http://127.0.0.1:8765/?theme=light)或[深色预览](http://127.0.0.1:8765/?theme=dark)。默认浅色、明天的播出准备；可切换深浅背景、暂停动效、停止日期自动切换。服务只绑定本机，预览需要时保留；不再需要时在启动终端Ctrl+C停止，不安装常驻服务。

使用 `?theme=dark` 可指定深色，`?channels=40` 用于展示布局验证。默认每个播出日期20频道，工作流6节点各有完整001—020，底部给出频道名称。40频道使用同一结构，编号延伸至040。
...
纯静态页面无需第三方依赖。`.openai/hosting.json` 仅保留静态目录元数据，内网运行不读取它，也不依赖 Sites 或任何云服务。真实状态服务与离线安装包目前只有方案，尚未实现或部署。
